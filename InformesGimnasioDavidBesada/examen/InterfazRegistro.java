...
public class InterfazRegistro extends JFrame {
	 ...
	 * Create the frame.
	 */
	public InterfazRegistro(InterfazPrincipal principal) {
	    this.intPrincipal = principal;
		isRegistrado = false;
	...
	public void registrar() {
	    try {
	        ...
	        Usuario nuevo = new Usuario(tfDNI.getText(), tfNombre.getText(), tfApellidos.getText(), edad);
	        
	        intPrincipal.addUsuario(nuevo); // <--- Línea clave
	        
	        JOptionPane.showMessageDialog(this, "Usuario registrado con éxito");
	        this.dispose(); // Cierra la ventana tras registrar
	...
	public boolean validar() {
	    boolean dniOk = tfDNI.getText().matches("\\d{8}[A-Z]");
	    boolean edadOk = false;
	    try {
	        int edad = Integer.parseInt(tfEdad.getText());
	        if (edad >= 0 && edad <= 100) edadOk = true;
	    } catch (Exception e) {}

	    return (dniOk && edadOk) || chkValPos.isSelected();
	}
