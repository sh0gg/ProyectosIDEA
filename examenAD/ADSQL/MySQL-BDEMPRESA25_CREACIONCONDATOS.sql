/*
en MySQL, o punto e coma (;) ao final dunha instrución SQL é obrigatorio para indicar  que a instrución anterior está completa e lista para ser executada.
*/
/*
comentarios: 
O Símbolo de Hash (#).   É o método de comentario de liña única máis sinxelo e específico de MySQL.
O Guión Dobre (-- ).  Este método é estándar en SQL (ANSI/ISO SQL), pero en MySQL require que estea seguido por un espazo en branco ou un carácter de control (como o final da liña). Se non hai espazo, MySQL ignora a instrución.
Comentarios de Bloque ou Multiliña: O Delimitador--  /* ... */
-- */
...
-- Borrar a base de datos se existe.
DROP DATABASE IF EXISTS BDEMPRESA25;
...
-- Desactivamos a comprobación de FKs temporalmente
-- Isto é crucial para poder borrar táboas en calquera orde e para a inserción circular.
...
SELECT 'Estrutura de táboas borrada (se existían).';

---

-- ######################################################################
-- 3. CREACIÓN DE TABOAS CON PK e UNIQUE
-- ENGINE=InnoDB é o motor por defecto e soporta Claves Alleas.
-- ######################################################################
...
-- el motor de almacenamiento por defecto es InnoDB por lo cual podemos supriermo ENGINE=InnoDB
...
CREATE TABLE LUGAR (
    ID INT AUTO_INCREMENT NOT NULL, -- Uso de AUTO_INCREMENT en lugar de IDENTITY(1,1)
...
-- FK_CHECKS xa está desactivado dende a Sección 1
-- ######################################################################

SELECT 'Desactivando restricións para a inserción de datos... (FK_CHECKS=0)';
