-- ######################################################################
-- 1. CREACIÓN DA BASE DE DATOS EN SQLITE (CONFIGURACIÓN)
-- ######################################################################

-- DESACTIVAMOS TEMPORALMENTE a comprobación de claves alleas.
-- Isto é crucial para poder borrar kas táboas EMPREGADO e DEPARTAMENTO,
-- xa que teñen unha dependencia circular (DEPARTAMENTO refire a EMPREGADO e viceversa).
PRAGMA foreign_keys = OFF;

-- ######################################################################
-- 2. DROP DE TABOAS SE EXISTEN
-- ######################################################################
DROP TABLE IF EXISTS EMPREGADO_PROXECTO;
DROP TABLE IF EXISTS EMPREGADOFIXO;
DROP TABLE IF EXISTS EMPREGADOTEMPORAL;
...
-- ######################################################################
-- 3. CREACIÓN DE TABOAS CON CONSTRAINTS NOMEADAS
-- ######################################################################
-- Activamos la comprobacion de claves foráneas 
PRAGMA foreign_keys = ON;

-- Nota sobre Claves Alleas (Foreign Keys):
-- En SQL (e SQLite), as restricións de Clave Allea (FOREIGN KEY) DEBEN ser definidas
-- na instrución CREATE TABLE. A cláusula CONSTRAINT Nome_Restricción úsase para nomealas explicitamente.
-- TÁBOA DEPARTAMENTO (O director debe existir en EMPREGADO, a FK engádese aquí)
CREATE TABLE DEPARTAMENTO (
    NumDepartamento INTEGER NOT NULL PRIMARY KEY,
    NomeDepartamento TEXT NOT NULL UNIQUE,  
    NSSDirector TEXT NOT NULL,
    -- FK do director (referencia a EMPREGADO).
    CONSTRAINT FK_DEPARTAMENTO_EMPLEADO FOREIGN KEY (NSSDirector) REFERENCES EMPREGADO(NSS)
);
...
CREATE TABLE EMPREGADO (
    ...
    -- Restrición CHECK xa nomeada no script orixinal (CK_SEXO)
    ...
    -- FK do Supervisor (Autoreferencia)
    CONSTRAINT FK_EMPLEADO_EMPLEADO FOREIGN KEY (NSSSupervisa) REFERENCES EMPREGADO(NSS)
);

-- TÁBOA PROXECTO
CREATE TABLE PROXECTO (
    NumProxecto INTEGER NOT NULL PRIMARY KEY,
    NomeProxecto TEXT NOT NULL UNIQUE,
    Lugar TEXT NOT NULL,
...
-- TÁBOA LUGAR
-- Para los campos autoincrementados se crea una tabla sqlite_sequence es una tabla interna especial que SQLitea automáticamente para gestionar la numeración de los campos definidos con el atributo AUTOINCREMENT.
--  No se debe crear ni modificar manualmente.
...
CREATE TABLE LUGAR (
    ...
    Lugar TEXT NOT NULL,
    CONSTRAINT UK_LUGAR UNIQUE(Num_departamento, Lugar),
...
-- SELECT 'Desactivando restricións para a inserción de datos... (PRAGMA foreign_keys = OFF)';
PRAGMA foreign_keys = OFF;

-- INSERTS EMPREGADO (Executado antes da creación da FK de DEPARTAMENTO para que haxa directores)
...
-- INSERTS DEPARTAMENTO (Teñen que ir despois de EMPREGADO, a pesar do PRAGMA OFF, para que as FKs do director funcionen. Xa se copiaron datos á táboa con FK.)
...
-- ######################################################################
-- 6. ACTIVACIÓN E VALIDACIÓN DE RESTRICCIÓNS
-- ######################################################################

-- SELECT 'Activando e validando restricións (FKs, CHECKs)...';

-- Reactiva a comprobación de FKs (valida a integridade dos datos que se acaban de inserir)
PRAGMA foreign_keys = ON;

-- SELECT 'Script completado. A base de datos BDEMPRESA25 está lista.';